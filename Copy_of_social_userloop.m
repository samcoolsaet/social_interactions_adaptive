...
TrialRecord.User.max_block = TrialRecord.User.button_progression_factor * 2 ...
    + TrialRecord.User.size_progression_factor;                             % last button active + at final size
TrialRecord.User.min_block = TrialRecord.User.button_progression_factor;    % at leat category progression factor because I never want to show 1 button again
...
% fixed constants
[TrialRecord.User.chasing_on, TrialRecord.User.grooming_on, TrialRecord.User.holding_on, ...
TrialRecord.User.mounting_on, TrialRecord.User.agent_on, TrialRecord.User.patient_on, ...
TrialRecord.User.categorizing, TrialRecord.User.agenting, TrialRecord.User.patienting, ...
TrialRecord.User.grooming, TrialRecord.User.chasing, TrialRecord.User.holding, ...
TrialRecord.User.mounting] = deal(false);                                   % always false for script to work
...
if TrialRecord.User.completed_stimuli == TrialRecord.User.blocksize             % if ze have completed a blocksize of stimuli, evaluate
    ...
    % reset all the completed stimuli
    % this way, the data of the used, but not completed ones stays in there
    [TrialRecord.User.structure(indexes_used_c_stimuli).c_success, TrialRecord.User.structure(indexes_used_c_stimuli).c_fails, ...
    TrialRecord.User.structure(indexes_used_c_stimuli).c_completed, TrialRecord.User.structure(indexes_used_a_stimuli).a_success, ...
    TrialRecord.User.structure(indexes_used_a_stimuli).a_fails, TrialRecord.User.structure(indexes_used_c_stimuli).a_completed, ...
    TrialRecord.User.structure(indexes_used_p_stimuli).p_success, TrialRecord.User.structure(indexes_used_p_stimuli).p_fails, ...
...
if TrialRecord.User.training_categorization || TrialRecord.User.training_agent_patient
    if TrialRecord.NextBlock > TrialRecord.User.max_block
        ...
        disp(['max block number reached' string(TrialRecord.User.max_c_progression_number)]);
    elseif TrialRecord.NextBlock < TrialRecord.User.min_block
        TrialRecord.NextBlock = TrialRecord.User.min_block;
        disp(['min block number reached' string(TrialRecord.User.min_c_progression_number)]);
    end
end
...
% are turned on 
if TrialRecord.User.button_progression
