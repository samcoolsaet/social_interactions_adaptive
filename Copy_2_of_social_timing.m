...
%     bystander_box = {[0.5 0.5 0.5], [0.5 0.5 0.5], standard_button_size, [x_axes(2) (y_center + 2*y_spacing)]};
    agent_box = {[0 1 1], [0 1 1], standard_button_size, loc_button_1};
    patient_box = {[1 0 1], [1 0 1], standard_button_size, loc_button_2};
    bystander_box = {[0.5 0.5 0.5], [0.5 0.5 0.5], standard_button_size, loc_button_3};
% else
%     engaging_box = { [1 1 1], [1 1 1], standard_button_size, [10 0] };
...
% determine which behavior when agent_patient
if contains(TrialRecord.User.structure(TrialRecord.User.struct_index).folder, 'chasing')
    category = 1;
elseif contains(TrialRecord.User.structure(TrialRecord.User.struct_index).folder, 'grooming')
    category = 2;
elseif contains(TrialRecord.User.structure(TrialRecord.User.struct_index).folder, 'mounting')
    category = 3;
elseif contains(TrialRecord.User.structure(TrialRecord.User.struct_index).folder, 'holding')
    category = 4;
end
...
if TrialRecord.User.training_agent_patient
    if category / TrialRecord.User.num_cats >= 1
        ...
        elseif TrialRecord.User.patienting
            ...
            bystander_box(3) = {wrong_button_size};
        elseif TrialRecord.User.bystanding
            agent_box(3) = {wrong_button_size};
            patient_box(3) = {wrong_button_size};            
        end
    end
...
if TrialRecord.CurrentTrialNumber > (1+TrialRecord.User.test_trial_counter)
    TrialRecord.User.reward_factors = linspace(1,0.7,5); % linspace(1,0.7,15)
    if TrialRecord.TrialErrors(end-TrialRecord.User.test_trial_counter)...
            == 0 && TrialRecord.User.reward_index < 6 ...
...
elseif TrialRecord.User.current_sum_buttons == 1
    if touch.Success
        dashboard(2, 'success!!! <3 ');
        trialerror(0);
...
elseif TrialRecord.User.bystanding & TrialRecord.User.current_sum_buttons ~= 1
    if touch.ChosenTarget == 3
        dashboard(2, 'success!!! <3 ');
        trialerror(0)
        TrialRecord.User.structure(TrialRecord.User.struct_index).b_success = TrialRecord.User.structure(TrialRecord.User.struct_index).b_success + 1;
        reward = true;
    else
        TrialRecord.User.structure(TrialRecord.User.struct_index).b_fails = TrialRecord.User.structure(TrialRecord.User.struct_index).b_fails + 1;       
        reward = false;
        if tc_answer.Success
            dashboard(2, 'no response');
            trialerror(1);
...
%     goodmonkey(reward_dur1, 'numreward', 3, 'pausetime', 1000, 'eventmarker', 90, 'nonblocking', 1); % 'numreward', 3, 'pausetime', 1000, 'eventmarker', 90, 'nonblocking', 1);
    goodmonkey(reward_dur1); % 'numreward', 3, 'pausetime', 1000, 'eventmarker', 90, 'nonblocking', 1);
...
total_c = sum(cellfun(@(x) x== 1 || x==2 || x==3 || x==4, TrialRecord.User.struct_conditions(:,:,1)), 'all');
total_a = sum(cellfun(@(x) x== 5, TrialRecord.User.struct_conditions(:,:,1)), 'all');
total_p = sum(cellfun(@(x) x== 6, TrialRecord.User.struct_conditions(:,:,1)), 'all');
total_b = sum(cellfun(@(x) x== 7, TrialRecord.User.struct_conditions(:,:,1)), 'all');

TrialRecord.User.c_structure_completion = sum([TrialRecord.User.structure.c_completed])/total_c;
TrialRecord.User.a_structure_completion = sum([TrialRecord.User.structure.a_completed])/total_a;
TrialRecord.User.p_structure_completion = sum([TrialRecord.User.structure.p_completed])/total_p;
TrialRecord.User.b_structure_completion = sum([TrialRecord.User.structure.b_completed])/total_b;

TrialRecord.User.structure_completion = sum([TrialRecord.User.structure.c_completed] + ...
    [TrialRecord.User.structure.a_completed] + [TrialRecord.User.structure.p_completed] + ...
    [TrialRecord.User.structure.b_completed])/ sum([total_c total_a total_p total_b]);

TrialRecord.User.completed_stimuli = sum([TrialRecord.User.structure.c_completed] + ...
    [TrialRecord.User.structure.a_completed] + [TrialRecord.User.structure.p_completed] + ...
