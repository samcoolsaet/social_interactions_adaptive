...
if (TrialRecord.User.random_condition_order_index...
    ...
    for i = 1:length(indexes_used_c_stimuli)
        used_c_conditions(end+1) = TrialRecord.User.structure(indexes_used_c_stimuli(i)).condition(1);
    end
    indexes_used_a_stimuli = find([TrialRecord.User.structure.a_completed]==1);
    ...
    conditions_already_completed = TrialRecord.User.random_condition_order(end-length(used_conditions)+1:end);
    steps_till_reset = TrialRecord.User.blocksize - TrialRecord.User.completed_stimuli;
    while ~restricted
        ...
        problem_vector = horzcat(conditions_already_completed, TrialRecord.User.random_condition_order(1:steps_till_reset));
